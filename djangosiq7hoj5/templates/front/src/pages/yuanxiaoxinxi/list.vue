...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>学校名称：</div>
        <el-input v-model="formSearch.schoolname" placeholder="学校名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>专业名称：</div>
        <el-input v-model="formSearch.specialname" placeholder="专业名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>学校类型：</div>
        <el-input v-model="formSearch.typeschool" placeholder="学校类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>硕士类型：</div>
        <el-input v-model="formSearch.degreename" placeholder="硕士类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('yuanxiaoxinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 10px","margin":"0px 10px 0 0","color":"#333","borderRadius":"2px","background":"#0062a530","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/yuanxiaoxinxiAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 10px","margin":"10px 0 0","borderColor":"#cbcbcb","background":"#fff","borderWidth":"0 0 1px","display":"flex","width":"100%","fontSize":"0","position":"relative","borderStyle":"dotted","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","alignItems":"center","color":"#666","flexWrap":"wrap","flex":"1","background":"none","display":"flex","fontSize":"14px","height":"auto"}'>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">年份:{{item.year}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">学校名称:{{item.schoolname}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">{{item.specialname}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">总分:{{item.total}}</div>
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.schoolname = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.schoolname != '') searchWhere.schoolname = '%' + this.formSearch.schoolname + '%';
        if (this.formSearch.specialname != '') searchWhere.specialname = '%' + this.formSearch.specialname + '%';
        if (this.formSearch.typeschool != '') searchWhere.typeschool = '%' + this.formSearch.typeschool + '%';
        if (this.formSearch.degreename != '') searchWhere.degreename = '%' + this.formSearch.degreename + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`yuanxiaoxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/yuanxiaoxinxiDetail', query: params});
      },
