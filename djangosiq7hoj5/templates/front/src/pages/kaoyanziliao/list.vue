...
<div>
	...
	<div class="list-preview" :style='{"padding":"0 7%","margin":"0px auto","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="category-3" :style='{"border":"0px solid #eee","padding":"0","boxShadow":"0px 0px 0px #eee","margin":"20px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"3"}'>
			...
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"40px","margin":"0 5px 0 0","objectFit":"cover","borderRadius":"50px","display":"inline-block","height":"40px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>资料名称：</div>
        <el-input v-model="formSearch.ziliaomingcheng" placeholder="资料名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>资料简介：</div>
        <el-input v-model="formSearch.ziliaojianjie" placeholder="资料简介" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('kaoyanziliao','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 10px","margin":"0px 10px 0 0","color":"#333","borderRadius":"2px","background":"#0062a530","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/kaoyanziliaoAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 10px","margin":"10px 0 0","borderColor":"#cbcbcb","background":"#fff","borderWidth":"0 0 1px","display":"flex","width":"100%","fontSize":"0","position":"relative","borderStyle":"dotted","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","alignItems":"center","color":"#666","flexWrap":"wrap","flex":"1","background":"none","display":"flex","fontSize":"14px","height":"auto"}'>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">{{item.ziliaomingcheng}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">资料类型:{{item.ziliaoleixing}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('ziliaoleixing/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'ziliaoleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.ziliaomingcheng != '') searchWhere.ziliaomingcheng = '%' + this.formSearch.ziliaomingcheng + '%';
        if (this.formSearch.ziliaojianjie != '') searchWhere.ziliaojianjie = '%' + this.formSearch.ziliaojianjie + '%';
        if (this.curFenlei != '全部') searchWhere.ziliaoleixing = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`kaoyanziliao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/kaoyanziliaoDetail', query: params});
      },
